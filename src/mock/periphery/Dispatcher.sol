...
import {RingSwapRouter} from "./RingSwapRouter.sol";
import {BytesLib} from "./BytesLib.sol";
import {Payments} from "./Payments.sol";
import {PaymentsImmutables} from "src/mock/periphery/PaymentsImmutables.sol";
import {NFTImmutables} from "src/mock/periphery/NFTImmutables.sol";
import {Callbacks} from "./Callbacks.sol";
import {Commands} from "src/mock/periphery/Commands.sol";
import {LockAndMsgSender} from "./LockAndMsgSender.sol";
import {ERC721} from "src/mock/periphery/ERC721.sol";
import {ERC1155} from "src/mock/periphery/ERC1155.sol";
import {ERC20} from "src/mock/periphery/ERC20.sol";
import {IAllowanceTransfer} from "src/mock/periphery/IAllowanceTransfer.sol";
import {ICryptoPunksMarket} from "src/mock/periphery/ICryptoPunksMarket.sol";
    ...
    LockAndMsgSender
{
    ...
    function dispatch(
        ...
        if (command < Commands.FOURTH_IF_BOUNDARY) {
            if (command < Commands.SECOND_IF_BOUNDARY) {
                ...
                if (command < Commands.THIRD_IF_BOUNDARY) {
                    if (command == Commands.SEAPORT_V1_5) {
                        ...
                        (uint256 value, bytes calldata data) = getValueAndData(
                            inputs
                        );
                        (success, output) = SEAPORT_V1_5.call{value: value}(
                            data
                        );
                        ...
                        bytes calldata data = inputs.toBytes(1);
                        (success, output) = LOOKS_RARE_V2.call{value: value}(
                            data
                        );
                        ...
                        (success, output) = CRYPTOPUNKS.call{value: value}(
                            abi.encodeWithSelector(
                                ICryptoPunksMarket.buyPunk.selector,
                                punkId
                            )
                        );
                        if (success)
                            ICryptoPunksMarket(CRYPTOPUNKS).transferPunk(
                                map(recipient),
                                punkId
                            );
                        ...
                        success = (ERC1155(token).balanceOf(owner, id) >=
                            minBalance);
                        if (!success)
                            output = abi.encodePacked(
                                InvalidOwnerERC1155.selector
                            );
                    ...
                    if (command == Commands.X2Y2_721) {
                        ...
                        (success, output) = ELEMENT_MARKET.call{value: value}(
                            data
                        );
                    } else {
            ...
            if (command == Commands.SEAPORT_V1_4) {
            ...
            } else if (command == Commands.FEW_V2_SWAP_EXACT_IN) {
                ...
                assembly {
                    ...
                    isOriginalOutput := calldataload(add(inputs.offset, 0xa0))
                }
                address[] calldata path = inputs.toAddressArray(3);
                address payer = payerIsUser ? lockedBy : address(this);
                fewV2SwapExactInput(
            ...
            } else if (command == Commands.FEW_V2_SWAP_EXACT_OUT) {
                // equivalent: abi.decode(inputs, (address, uint256, uint256, bytes, bool, bool))
                address recipient;
                uint256 amountOut;
                uint256 amountInMax;
            ...
            } else if (command == Commands.RING_V2_SWAP_EXACT_IN) {
                // equivalent: abi.decode(inputs, (address, uint256, uint256, bytes, bool, bool))
                address recipient;
                uint256 amountIn;
                uint256 amountOutMin;
            ...
            } else if (command == Commands.RING_V2_SWAP_EXACT_OUT) {
                // equivalent: abi.decode(inputs, (address, uint256, uint256, bytes, bool, bool))
                address recipient;
                uint256 amountOut;
                uint256 amountInMax;
                ...
                // placeholder area for commands 0x27-0x3f
                revert InvalidCommandType(command);
            }
        }
    }
